...
def solve_system(A,b,method = 'QR'):
    '''
    Solve Ac = b using the given method. Default is build in numpy QR
    '''
    if method == 'QR':
        Q,R= np.linalg.qr(A)
        coefficients = solve_qr(Q,R,b)
    elif method == 'Cholesky':
        ATA = np.matmul(np.transpose(A),A)
        L = np.linalg.cholesky(ATA)
        ATb = np.matmul(np.transpose(A),b)
        coefficients = forward_sub(L,ATb)
    else:
        raise ValueError('Invalid Method for solving system given.')
